...
class GridMap2D {
 public:
  ...
  Eigen::Vector2d getESDFMax_and_Min() const;
...
// 人工势场地图最大最小值
// double esdf_map_max,esdf_map_min;
template <typename T>
...
template <typename T>
Eigen::Vector2d GridMap2D<T>::getESDFMax_and_Min() const
{
  auto copy_data = map_data_;
  cv::Mat grid_mat(cell_num_[1], cell_num_[0],
                   CV_MAKETYPE(cv::DataType<T>::type, 1), copy_data.data());
  ...
  return Eigen::Vector2d(esdf_map_min,esdf_map_max);  
}
