/*
 * @Author: your name
 * @Date: 2023-04-04 16:16:43
 * @LastEditTime: 2023-05-14 14:07:46
 * @LastEditors: your name
 * @Description: 
 * @FilePath: /Robot_Formation/src/robot_gmapping/src/occupancy_grid/occupancy_grid.cpp
...
void OccupancyGrid::PublishMap()
{
    ...
    // 固定列, 优先对行进行遍历
    for (long unsigned int j = 0; j < map_.info.height; j++)
    {
        for (long unsigned int i = 0; i < map_.info.width; i++)
        {
            // 二维坐标转成一维坐标
...
{
  ros::init(argc, argv, "make_occupancy_grid_map");
