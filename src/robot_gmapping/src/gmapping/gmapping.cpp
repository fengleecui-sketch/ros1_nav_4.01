...
namespace gmapping
{
  ...
  GMapping::GMapping() : private_node_("~")
  {
    ...
    // 获取激光雷达发布的话题 laser_scan是模拟仿真的激光雷达  scan是真正的雷达
    private_node_.param<std::string>("laserType",laserType,"laser_scan");
    laser_scan_subscriber_ = node_handle_.subscribe(
        laserType, 1, &GMapping::ScanCallback, this);

    map_publisher_ = node_handle_.advertise<nav_msgs::OccupancyGrid>("gmapping", 1, true);
