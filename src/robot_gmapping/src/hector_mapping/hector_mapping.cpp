...
namespace hector_mapping
{
  ...
  HectorMapping::HectorMapping() : private_node_("~")
  {
    ...
    laser_scan_subscriber_ = node_handle_.subscribe(
        laserType, 1, &HectorMapping::ScanCallback, this);

    map_publisher_ = node_handle_.advertise<nav_msgs::OccupancyGrid>("local_map", 1, true);
  ...
  {
    // 获取launch文件中的参数
    private_node_.param("maptype/localmap",map_type,false);
    private_node_.param("local_map/xmin", xmin_,-40.0);
    private_node_.param("local_map/ymin", ymin_,-40.0);
    private_node_.param("local_map/xmax", xmax_,40.0);
    private_node_.param("local_map/ymax", ymax_,40.0);
    private_node_.param("local_map/resolution", resolution_,0.05);
  ...
  void HectorMapping::ScanCallback(const sensor_msgs::LaserScan &scan)
  {
    ...
    // 输出地图的 x y坐标
    // std::cout<<hector_map_->getSizeX()<<"  "<<hector_map_->getSizeY()<<std::endl;
  }
  ...
  void HectorMapping::PublishMap()
  {
    ...
    for (int i = 0; i < size; ++i)
    {
      ...
      {
        // 当遇到障碍物设定为100
        if (hector_map_->isOccupied(i))
        {
          data[i] = 100;
        }
        // 没遇到障碍物都设定为0
        else if (hector_map_->isFree(i))
        {
          data[i] = 0;
        }
      }
...
{
    ros::init(argc, argv, "make_hector_map");
