...
namespace dyn_planner
{
  ...
  void DynPlannerManager::setPathFinder(const Hybrid_astar::Ptr &finder)
  {
    path_finder_ = finder;
  }
  ...
  bool DynPlannerManager::checkTrajCollision()
  {
    ...
    {
      Eigen::Vector2d pos = traj_pos_.evaluateDeBoor(t);
  ...
  bool DynPlannerManager::generateTrajectory(Eigen::Vector2d start_pt, Eigen::Vector2d start_vel,
                                             Eigen::Vector2d start_acc, Eigen::Vector2d end_pt, Eigen::Vector2d end_vel)
  {
    ...
    oriworldPath.clear();
    oriworldPath = path_finder_->getKinoTraj(0.01);
    ...
    Eigen::MatrixXd control_pts; // x, y轴的控制点初值

    NonUniformBspline::getControlPointEqu3(samples, ts, control_pts);

    vector<Vector2d> tempath;
    optworldPath.clear();
    // cout<<"control_pts: row:"<<control_pts.rows()<<"col:"<<control_pts.cols()<<endl;
    // for(int i=0;i<control_pts.rows();i++)
    // {
    //   tempath.push_back(Vector2d(control_pts(i,0),control_pts(i,1)));
    // }
    for(int i=0;i<samples.cols();i++)
    {
      tempath.push_back(Vector2d(samples(0,i),samples(1,i)));
    }
    optworldPath = tempath;
