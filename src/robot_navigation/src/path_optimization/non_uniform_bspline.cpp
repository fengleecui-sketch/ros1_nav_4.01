...
NonUniformBspline::NonUniformBspline(Eigen::MatrixXd points, int order, double interval_, bool zero)
{
    ...
    // cout<<"this->p_:"<< this->p_<<endl;

    // cout<<"control_points rows:"<<points.rows()<<endl;
    // cout<<"control_points cols:"<<points.cols()<<endl;


    control_points_ = points;
    // n-1个控制点
    this->n_ = points.rows() - 1; // 控制点Q: Q0, Q1, ... , Qn
    // cout<<"this->n_"<<this->n_<<endl;
    ...
    if (zero)
    {
        x0_ = (1 / 6.0) * (control_points_.row(0) + 4 * control_points_.row(1));
        v0_ = (1 / 2.0 / interval_) * (- control_points_.row(0));
        a0_ = (1 / interval_ / interval_) * (control_points_.row(0) - 2 * control_points_.row(1));
...
Eigen::Vector2d NonUniformBspline::evaluateDeBoor(double t)
{
...
Eigen::MatrixXd NonUniformBspline::getDerivativeControlPoints()
{
    ...
    Eigen::MatrixXd ctp = Eigen::MatrixXd::Zero(control_points_.rows() - 1, 2);
...
bool NonUniformBspline::checkFeasibility(bool show)
{
    ...
    for (int i = 0; i < P.rows() - 1; ++i)
    {
        ...
        Eigen::Vector2d vel = p_ * (P.row(i + 1) - P.row(i)) / (u_(i + p_ + 1) - u_(i + 1));
...
void NonUniformBspline::recomputeInit()
{
    ...
    /* solve */
    Eigen::Vector2d px = A.colPivHouseholderQr().solve(bx);
    Eigen::Vector2d py = A.colPivHouseholderQr().solve(by);
...
void NonUniformBspline::getControlPointEqu3(Eigen::MatrixXd samples, double ts, Eigen::MatrixXd &control_pts)
{
    ...
    // write A
    Eigen::VectorXd prow(2), vrow(2), arow(2);
    prow << 1, 4;
    vrow << -1, 0;
    arow << 1, -2;
    ...
    for (int i = 0; i < K + 2; ++i)
        A.block(i, i, 1, 2) = prow.transpose(); // 每个采样点的pos

    A.block(K + 2, 0, 1, 2) = A.block(K + 3, K + 1, 1, 2) = vrow.transpose(); // 起点和终点的vel
    A.block(K + 4, 0, 1, 2) = arow.transpose(); // 起点的acc
    ...
    // 超定方程组, 使用最小二乘求解 Ax = b, px py为求解出的控制点
    Eigen::VectorXd px = A.colPivHouseholderQr().solve(bx);
    Eigen::VectorXd py = A.colPivHouseholderQr().solve(by);

    // convert to control pts
    control_pts.resize(K + 4, 2);
    control_pts.col(0) = px;
    control_pts.col(1) = py;
}

void NonUniformBspline::BsplineParameterize(const double &ts, const vector<Eigen::Vector2d> &point_set,
                                            ...
                                            Eigen::MatrixXd &ctrl_pts)
{
    ...
    for (int i = 0; i < K + 2; ++i)
        A.block(i, i, 1, 2) = prow.transpose();
    A.block(K + 2, 0, 1, 2) = A.block(K + 3, K + 1, 1, 2) = vrow.transpose();
    A.block(K + 4, 0, 1, 2) = A.block(K + 5, K + 1, 1, 2) = arow.transpose();
    ...
    // write b
    Eigen::VectorXd bx(K + 6), by(K + 6);
    for (int i = 0; i < K + 2; ++i)
    {
        bx(i) = point_set[i](0), by(i) = point_set[i](1);
    }
...
double NonUniformBspline::getLength()
{
    ...
    double tm, tmp;
    getTimeSpan(tm, tmp);
    Eigen::Vector2d p_l = evaluateDeBoor(tm), p_n;
...
void NonUniformBspline::getMeanAndMaxVel(double &mean_v, double &max_v)
{
    ...
    {
        Eigen::Vector2d v2d = vel.evaluateDeBoor(t);
        double vn = v2d.norm();
...
void NonUniformBspline::getMeanAndMaxAcc(double &mean_a, double &max_a)
{
    ...
    for (double t = tm; t <= tmp; t += 0.01)
    {
        Eigen::Vector2d a2d = acc.evaluateDeBoor(t);
        double an = a2d.norm();
