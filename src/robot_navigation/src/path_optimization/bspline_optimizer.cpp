...
namespace dyn_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    // 控制点发布
    smoothPathpub = nh.advertise<nav_msgs::Path>("/smooth",10);
    // 
    obstaclePathpub = nh.advertise<nav_msgs::Path>("/obstacle",10);
  }
                                            ...
                                            vector<Eigen::Vector2d> &gradient)
  {
    ...
    // jerk三阶导数
    Eigen::Vector2d jerk; // Σ|Qi+3 - 3 * Qi+2 + 3 * Qi+1 - Qi|^2
                                             ...
                                             vector<Eigen::Vector2d> &gradient)
  {
    ...
    {
      Eigen::Vector2d vi = q[i + 1] - q[i]; // Vi = 1 / ts * (Qi+1 - Qi)
      for (int j = 0; j < 2; j++)           // x, y, z轴
      {
    ...
    {
      Eigen::Vector2d ai = q[i + 2] - 2 * q[i + 1] + q[i]; // Ai = 1 / ts * (Vi+1 - Vi) = 1 / ts^2 * (Qi+2 - 2* Qi+1 + Qi)
      for (int j = 0; j < 2; j++)
      {
  ...
  void BsplineOptimizer::combineCost(const std::vector<double> &x, std::vector<double> &grad, double &f_combine)
  {
    ...
    vector<Eigen::Vector2d> q; // q为优化后的控制点(优化变量+固定变量), 用来评估此次迭代后的cost
    ...
    {
      // 把二维点转换成二维的向量然后放入容器中
      Eigen::Vector2d qi(x[2 * i], x[2 * i + 1]); // 优化变量x为std::vector<double>, 需要转化为Vector3d格式
      q.push_back(qi);
    }
    ...
    vector<Eigen::Vector2d> g_smoothness, g_distance, g_feasibility, g_endpoint; // 每个控制点的梯度向量
  ...
  void BsplineOptimizer::PublishPath(ros::Publisher pathPublish, std::vector<Eigen::Vector2d> path)
  {
