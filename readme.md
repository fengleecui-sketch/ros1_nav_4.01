...
# 2) 启动 Gazebo + robot
gnome-terminal --title="gazebo_start" -- bash  -c "source ~/ros1_slam/devel/setup.bash; roslaunch tri_steer_gazebo carto_construct_map.launch"
# 等待 Gazebo 节点起来（/gazebo/model_states 出现再继续）
until rostopic list 2>/dev/null | grep -q "/gazebo/model_states"; do
  sleep 0.5
done
# 3) 启动 teleop + robot
gnome-terminal -t "teleop_start" -- bash -lc "source ~/ros1_slam/devel/setup.bash; roslaunch tri_steer_keyboard keyboard_tri_steer.launch"
...
# 2) 启动 Gazebo + robot
gnome-terminal --title="gazebo_start" -- bash -c "source ~/ros1_slam/devel/setup.bash; roslaunch tri_steer_gazebo sim_gazebo_rviz.launch"
...
# 6) 运动规划节点
gnome-terminal -t "motion Plan start" -- bash -lc "source ~/ros1_slam/devel/setup.bash; roslaunch robot_navigation motionPlan_sim.launch"
# 7）局部规划pid算法
gnome-terminal -t "PID_control" -- bash -c "source ~/ros1_slam/devel/setup.bash; roslaunch robot_pid_local_planner Omnidirectional_PID.launch; exec bash"
# 解析carto算法发布的tf变换到机器人
gnome-terminal -t "carto_odom_start" -- bash -lc "source ~/ros1_slam/devel/setup.bash; roslaunch carto_tf_to_odom carto_tf_to_odom.launch; exec bash"
# 9) 启动并发布机器人真值里程计消息
gnome-terminal -t "truth Odom start" -- bash -lc "source ~/ros1_slam/devel/setup.bash; roslaunch robot_locatization truth_odometry.launch;exec bash"
# 10）订阅控制消息
