...
# 2) 启动 Gazebo + robot
gnome-terminal --title="gazebo_start" -- bash -c "source ~/Gitdesktop/ros1_nav2026/devel/setup.bash; roslaunch tri_steer_gazebo sim_gazebo_carto.launch"

sleep 3

# 3) 雷达点云反转成水平车体下的point-cloud2 暂时无用
gnome-terminal -t "laser to cloud" -- bash -lc "source ~/Gitdesktop/ros1_nav2026/devel/setup.bash; roslaunch read_laser_data laser_to_cloud.launch; exec bash"
...
# 5）启动地图节点
#gnome-terminal -t "map start" -- bash -lc "source ~/Gitdesktop/ros1_nav2026/devel/setup.bash; roslaunch robot_costmap map_deal.launch;exec bash"
# 6) 运动规划节点
